...
@section('navbar', 'Déposer un projet')

@section('title', 'Espace Societe')

@section('content')
  <h1 class="text-xs-center">Déposer un Projet</h1>
  <div class="row">
    <div class="col-md-6">
      @if (count($errors) > 0)
        ...
        <tbody>
          @foreach ($projets as $projet)
            <tr>
              <th>{{ $projet->id }}</th>
              <td>{{ $projet->titre }}</td>
              <td><a href="{{ url('societe/editer') }}/{{ $projet->id }}" class="text-success">Éditer</a></td>
              <td><a href="{{ url('societe/supprimer') }}/{{ $projet->id }}" class="text-success">Supprimer</a></td>
            </tr>
          @endforeach
        </tbody>
      </table>
